  ...
  for (let idx = 0; idx < models.length; idx++) {
    ...
    for (let idy = 0; idy < models.length; idy++) {
      ...
      if (distance < ACTIVE_NEIGHBOR_RADIUS) {
        ...
        alignment.add(models[idy].getMesh().getWorldDirection());
    ...
    cohesion.multiplyScalar(-1.0/neighbors).add(activePos).multiplyScalar(-1.0);
    ...
    models[idx].updateFromVector(separation.add(cohesion).add(alignment).add(surface));
  }
